// lib/features/records/data/repositories/firestore_record_repository.dart
...
class FirestoreRecordRepository {
  ...
  Future<void> saveRecord(RecordModel record) async {
      ...
      if (key.length <= 50) {
        if (value is String && value.length > 500) {
          sanitizedMetadata[key] =
              value.substring(0, 500); // Truncate huge strings
        } else {
    ...
    await _recordsCollection
        .doc(sanitizedRecord.id)
        .set(sanitizedRecord.toFirestore());
  }
